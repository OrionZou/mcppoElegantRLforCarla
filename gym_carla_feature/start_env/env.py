...
class CarlaEnv(gym.Env):
    ...
    def __init__(self, params=defalt_config):
        ...
        if 'orgin_state' in self.obs_space_type:
            observation_space_dict = {
                'orgin_state': spaces.Box(low=-1000, high=1000, shape=(9,), dtype=np.float32)
            }
        ...
        elif params['task_mode'] == 'fixedroute':
    ...
    def reset(self):
        ...
        self.his_act_deque = deque(maxlen=(10 + 1))
    ...
    def _get_obs(self):
        ...
        self.his_act_deque.append([act_control.throttle - act_control.brake, act_control.steer, ego_trans.rotation.yaw])
        ahead_wp_idx = 0
        ...
        v = np.array([v.x, v.y])
        speed_lon = np.dot(v, ego_vector)
        speed_lat = np.cross(v, ego_vector)
        ...
        state = np.array([lateral_dis,  # rough distance from lane center to ego
                          ...
                          speed_lon,  # longitudinal speed
                          speed_lat,  # lateral speed
    ...
    def _get_reward(self, obs):
        ...
        # cost for too fast
        # r_speed_lon = lspeed_lon / self.desired_speed - max(0., (lspeed_lon - self.desired_speed) / (
        #         0.5 * self.desired_speed))
    ...
    def _get_info(self, obs, r_info):
        ...
        info['acc'] = 3. * control.throttle - 8. * control.brake
