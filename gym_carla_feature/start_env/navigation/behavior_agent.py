...
""" This module implements an navigation that roams around a track following random
waypoints and avoiding other vehicles. The navigation also responds to traffic lights,
...
from gym_carla_feature.start_env.navigation.local_planner_behavior import LocalPlanner, RoadOption
...
from gym_carla_feature.start_env.navigation.types_behavior import Cautious, Aggressive, Normal
from gym_carla_feature.start_env.misc import get_speed, positive


class BehaviorAgent(Agent):
    """
    BehaviorAgent implements an navigation that navigates scenes to reach a given
    target destination, by computing the shortest possible path to it.
    This navigation can correctly follow traffic signs, speed limitations,
    ...
    behaviors, the navigation can also keep safety distance from a car in front of it
    by tracking the instantaneous time to collision and keeping it in a certain range.
    Finally, different sets of behaviors are encoded in the navigation, from cautious
    ...
    def __init__(self, vehicle, ignore_traffic_light=True, behavior='normal'):
        ...
        Constructor method.
            ...
            :param behavior: type of navigation to apply
        """
    ...
    def update_information(self, vehicle, speed_limit=None):
        """
        ...
        self.speed_limit = vehicle.get_speed_limit() if speed_limit is None else speed_limit
        self._local_planner.set_speed(self.speed_limit * 3.6)
    ...
    def set_global_plan(self, route, clean=True):
        self._local_planner.set_global_plan(route, clean=clean)

    def is_empty_plan(self):
        if len(self._local_planner.waypoints_queue) < 15:
            return True
        else:
            return False
    ...
    def reroute(self, spawn_points):
        ...
        It finds a new target and computes another path to reach it.
            ...
            :param spawn_points: list of possible destinations for the navigation
        """
    ...
    def traffic_light_manager(self, waypoint):
        ...
        ignore it and go around this issue, until the car is near a new one.
            ...
            :param waypoint: current waypoint of the navigation
        """
    ...
    def _overtake(self, location, waypoint, vehicle_list):
        ...
        This method is in charge of overtaking behaviors.
            ...
            :param location: current location of the navigation
            :param waypoint: current waypoint of the navigation
