...
class ReplayBuffer:
    def __init__(self, max_len, state_dim, action_dim, reward_dim,
                 if_on_policy, if_per, if_discrete_action=False,
                 if_rnn=False,
                 hidden_state_dim=0,
                 if_gpu=False):
        ...
        self.if_per = if_per
        self.if_discrete_action = if_discrete_action
        self.if_gpu = if_gpu
        self.if_rnn = if_rnn
        ...
        if self.if_gpu:
            ...
            if self.if_rnn:
                self.buf_hidden_state = torch.empty((max_len, hidden_state_dim), dtype=torch.float32,
                                                    device=self.device)
                self.buf_cell_state = torch.empty((max_len, hidden_state_dim), dtype=torch.float32, device=self.device)
        else:
            ...
            if self.if_rnn:
                self.buf_hidden_state = np.empty((max_len, hidden_state_dim), dtype=np.float32)
                self.buf_cell_state = np.empty((max_len, hidden_state_dim), dtype=np.float32)

    def append_buffer(self, state, action, reward, gamma, hidden_state=None, cell_state=None):  # CPU array to CPU array
        if self.if_gpu:
            ...
            if self.if_rnn:
                hidden_state = torch.as_tensor(hidden_state, dtype=torch.float32, device=self.device)
                cell_state = torch.as_tensor(cell_state, dtype=torch.float32, device=self.device)
        self.buf_state[self.next_idx] = state
        if self.if_discrete_action:
            self.buf_action[self.next_idx] = action
        else:
            self.buf_action[self.next_idx] = action[:self.action_dim] if action.size > self.action_dim else action
        self.buf_reward[self.next_idx] = reward
        self.buf_gamma[self.next_idx] = gamma
        if self.if_rnn:
            self.buf_hidden_state[self.next_idx] = hidden_state
            self.buf_cell_state[self.next_idx] = cell_state
        if self.if_per:
    ...
    def extend_buffer(self, state, action, reward, gamma, hidden_state=None, cell_state=None):  # CPU array to CPU array
        if self.if_gpu:
            state = torch.as_tensor(state, dtype=torch.float32, device=self.device)
            action = torch.as_tensor(action, dtype=torch.float32, device=self.device)
            reward = torch.as_tensor(reward, dtype=torch.float32, device=self.device)
        ...
        if next_idx > self.max_len:
            if next_idx > self.max_len:
                ...
                if self.if_rnn:
                    self.buf_hidden_state[self.next_idx:self.max_len] = hidden_state[:self.max_len - self.next_idx]
                    self.buf_cell_state[self.next_idx:self.max_len] = cell_state[:self.max_len - self.next_idx]
            self.if_full = True
            ...
            if self.if_rnn:
                self.buf_hidden_state[0:next_idx] = hidden_state[-next_idx:]
                self.buf_cell_state[0:next_idx] = cell_state[-next_idx:]
        else:
            ...
            if self.if_rnn:
                self.buf_hidden_state[self.next_idx:next_idx] = hidden_state
                self.buf_cell_state[self.next_idx:next_idx] = cell_state
        self.next_idx = next_idx

    def sample_batch(self, batch_size) -> tuple:  # todo for rnn
    ...
    def sample_latest(self, size):
        sz = self.now_len if self.now_len < size else size

        if self.if_rnn:
            return (torch.as_tensor(self.buf_reward[self.now_len-sz:self.now_len], device=self.device),
                    torch.as_tensor(self.buf_gamma[self.now_len-sz:self.now_len], device=self.device),
                    torch.as_tensor(self.buf_action[self.now_len-sz:self.now_len], device=self.device),
                    torch.as_tensor(self.buf_state[self.now_len-sz:self.now_len], device=self.device),
                    torch.as_tensor(self.buf_hidden_state[self.now_len-sz:self.now_len], device=self.device),
                    torch.as_tensor(self.buf_cell_state[self.now_len-sz:self.now_len], device=self.device)
                    )
        else:
            return (torch.as_tensor(self.buf_reward[self.now_len-sz:self.now_len], device=self.device),
                    torch.as_tensor(self.buf_gamma[self.now_len-sz:self.now_len], device=self.device),
    ...
    def sample_all(self) -> tuple:
        ...
        if self.if_rnn:
            return (torch.as_tensor(self.buf_reward[:self.now_len], device=self.device),
                    ...
                    torch.as_tensor(self.buf_hidden_state[:self.now_len], device=self.device),
                    torch.as_tensor(self.buf_cell_state[:self.now_len], device=self.device)
                    )
        else:
            return (torch.as_tensor(self.buf_reward[:self.now_len], device=self.device),
                    torch.as_tensor(self.buf_gamma[:self.now_len], device=self.device),
...
class ReplayBufferMP:
    def __init__(self, max_len, state_dim, action_dim, reward_dim,
                 if_on_policy, if_per, if_discrete_action,
                 rollout_num,
                 if_rnn=False, hidden_state_dim=0,
                 if_gpu=False,
                 ):
        ...
        if self.if_on_policy:
            if_per = False
        self.buffers = [ReplayBuffer(_max_len,
                                     state_dim,
                                     action_dim,
                                     reward_dim,
                                     if_on_policy,
                                     if_per,
                                     if_discrete_action=if_discrete_action,
                                     if_rnn=if_rnn,
                                     hidden_state_dim=hidden_state_dim,
                                     if_gpu=self.if_gpu)
                        for _ in range(rollout_num)]

    def extend_buffer(self, state, action, reward, gamma, i, hidden_state=None, cell_state=None):
        if self.if_rnn:
            self.buffers[i].extend_buffer(state, action, reward, gamma, hidden_state, cell_state)
        else:
    ...
    def sample_latest(self, size):
        sz = size // self.rollout_num
        list_items = []
        list_items.append(self.buffers[0].sample_latest(sz + (size % self.rollout_num)))
        for i in range(1, self.rollout_num):
            list_items.append(self.buffers[i].sample_batch(sz))
        return [torch.cat([item[i] for item in list_items], dim=0)
                for i in range(len(list_items[0]))]
