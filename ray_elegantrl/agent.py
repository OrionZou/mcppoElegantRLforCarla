...
# class AgentConstriantPPO2(AgentPPO2):
#     def __init__(self, args=None):
#         super().__init__(args)
#         # AgentPPO is an on policy DRL algorithm
#         self.if_on_policy = True
...
#                 mean_kl = torch.distributions.kl_divergence(tar_act.get_distribution(state),
#                                                             self.act.get_distribution(state)).mean()
#                 obj_surrogate = -obj_surrogate1.mean(dim=0) + self.beta * mean_kl
#
...
#
#         self.update_record(obj_a=obj_surrogate.item(),
#                            obj_c0=obj_critics[0].item(),
#                            obj_c1=obj_critics[1].item(),
#                            obj_c2=obj_critics[2].item(),
#                            w0=self.weights[0].item(),
#                            w1=self.weights[1].item(),
#                            w2=self.weights[2].item(),
#                            mean_kl=100 * kl.mean().item(),
#                            # a_std=self.act.a_std_log.exp().mean().item(),
#                            entropy=(-obj_entropy.item()),
#                            a0_avg=buf_action[:, 0].mean().item(),
...
class AgentHCPPO2(AgentPPO2):
    ...
    def init(self, net_dim, state_dim, action_dim, reward_dim=1, if_per=False, device=None):
        ...
        # self.target_entropy = -action_dim
        self.state_dim = state_dim
        self.c_action_dim = action_dim - 1
        self.d_action_dim = self.discrete_degree ** self.c_action_dim
        self.cri_d = CriticAdv(state_dim, net_dim).to(self.device)
        self.cri_c = CriticAdv(state_dim + self.d_action_dim, net_dim).to(self.device)
        self.act_d = ActorDiscretePPO(net_dim, state_dim, self.d_action_dim).to(self.device)
        self.act_c = ActorPPO(net_dim, state_dim + self.d_action_dim, self.c_action_dim).to(self.device)
        if self.save_path is not None:
            self.load_model(self.save_path)
        self.act = [self.act_c, self.act_d]
        self.cri_c_optimizer = torch.optim.Adam(params=self.cri_d.parameters(), lr=self.learning_rate)
    ...
    def select_action(state, policy, explore_rate=1.) -> np.ndarray:
        ...
        if rd.rand() < explore_rate:  # epsilon-greedy
            d_action = policy[1].get_action(states)
            tmp = torch.nn.functional.one_hot(policy[1].get_action(states), 9)
            c_action = policy[0].get_action(torch.cat((states, tmp), dim=1))
            action = torch.cat((c_action, d_action.unsqueeze(dim=1)), 1)
        else:
            d_action = policy[1](states)
            tmp = torch.nn.functional.one_hot(policy[1].get_action(states), 9)
            c_action = policy[0](torch.cat((states, tmp), dim=1))
            action = torch.cat((c_action, d_action.unsqueeze(dim=1)), 1)
        if action.isnan().any():
            print(action)
        return action[0].detach().numpy()
    ...
    def update_net(self, buffer, _target_step, batch_size, repeat_times=4) -> (float, float):
        ...
        with torch.no_grad():
            ...
            if self.train_model in ['mix', 'discrete']:
                buf_d_action = buf_action[:, -1].unsqueeze(dim=1)
                buf_d_value = torch.cat(
                    [self.cri_d(buf_state[i:i + bs]) for i in range(0, buf_state.size(0), bs)], dim=0)
                buf_r_d_ret, buf_d_adv = self.compute_reward(buf_len, buf_reward, buf_mask, buf_d_value)
                buf_d_logprob = self.act_d.compute_logprob(buf_state, buf_d_action).unsqueeze(dim=1)
            if self.train_model in ['mix', 'continues']:
                buf_tmp_state = torch.cat(
                    (buf_state, torch.nn.functional.one_hot(buf_action[:, -1].type(torch.long), self.d_action_dim)),
                    dim=1)
                buf_c_value = torch.cat(
                    [self.cri_c(buf_tmp_state[i:i + bs]) for i in range(0, buf_tmp_state.size(0), bs)], dim=0)
                buf_r_c_ret, buf_c_adv = self.compute_reward(buf_len, buf_reward, buf_mask, buf_c_value)
                buf_c_action = buf_action[:, :-1]
                buf_c_logprob = self.act_c.compute_logprob(buf_tmp_state, buf_c_action).unsqueeze(dim=1)
            tar_act_d = deepcopy(self.act_d)
            tar_cri_d = deepcopy(self.cri_d)
            tar_act_c = deepcopy(self.act_c)
            tar_cri_c = deepcopy(self.cri_c)
        ...
        if self.train_model in ['mix', 'discrete']:
            ...
            for _ in range(int(repeat_times * buf_len / batch_size)):
                ...
                d_action = buf_d_action[indices]
                logprob = buf_d_logprob[indices]
                r_ret = buf_r_d_ret[indices]
                adv = buf_d_adv[indices]
        ...
        if self.train_model in ['mix', 'continues']:
            ...
            for _ in range(int(repeat_times * buf_len / batch_size)):
                indices = torch.randint(buf_len - 1, size=(batch_size,), requires_grad=False, device=self.device)
                state = buf_tmp_state[indices]
                c_action = buf_c_action[indices]
                logprob = buf_c_logprob[indices]
                r_ret = buf_r_c_ret[indices]
                adv = buf_c_adv[indices]
...
#             self.act.append(ActorPPOBeta(net_dim, state_dim, action_dim, self.if_use_dn).to(self.device))
#             self.act.append(AgentPPO(net_dim, state_dim, action_dim, self.if_use_dn).to(self.device))
#         else:
#             self.act.append(AgentPPO(net_dim, state_dim, action_dim, self.if_use_dn).to(self.device))
#             self.act.append(ActorPPOBeta(net_dim, state_dim, action_dim, self.if_use_dn).to(self.device))
#         for i in range(len(self.act)):
#             self.act_optimizer.append(torch.optim.Adam(params=self.act[i].parameters(), lr=self.learning_rate))
#         self.criterion = torch.nn.SmoothL1Loss()
#         self.iter_index = 0
#         assert if_per is False  # on-policy don't need PER
#
...
#        """
#         states = torch.as_tensor((state,), dtype=torch.float32).detach_()
#         index = 0 if np.random.rand() > 0.5 else 1
#         action = policy[index].get_action(states)[0]
#         return action.detach().numpy()
#         # return np.hstack((index,action.detach().numpy()))
#
#     def update_net(self, buffer, _target_step, batch_size, repeat_times=4) -> (float, float):
#         buffer.update_now_len_before_sample()
#         buf_len = buffer.now_len  # assert buf_len >= _target_step
...
#             tar_act = []
#             for i in range(len(self.act)):
#                 buf_logprob.append(self.act[i].compute_logprob(buf_state, buf_action).unsqueeze(dim=1))
#                 tar_act.append(deepcopy(self.act[i]))
#             del buf_reward, buf_mask
#
#         '''PPO: Surrogate objective of Trust Region'''
#         obj_critic = None
...
#                 logprob = buf_logprob[i][indices]
#                 new_logprob = act.compute_logprob(state, action).unsqueeze(dim=1)  # it is obj_actor
#                 ratio = (new_logprob - logprob).exp()
#                 obj_surrogate1 = adv * ratio
#                 if self.objective_type == 'clip':
#                     obj_surrogate2 = adv * ratio.clamp(1 - self.ratio_clip, 1 + self.ratio_clip)
#                     obj_surrogate = -torch.min(obj_surrogate1, obj_surrogate2).mean()
#                 elif self.objective_type == 'kl':
#                     mean_kl = torch.distributions.kl_divergence(tar_act[i].get_distribution(state),
#                                                                 act.get_distribution(state)).mean()
#                     obj_surrogate = -obj_surrogate1.mean() + self.beta * mean_kl
#                 obj_entropy = (new_logprob.exp() * new_logprob).mean()  # policy entropy
#                 obj_actor = obj_surrogate + obj_entropy * self.lambda_entropy
#
#                 self.act_optimizer[i].zero_grad()
#                 obj_actor.backward()
#                 torch.nn.utils.clip_grad_norm_(self.act[i].parameters(), 'inf')
#                 self.act_optimizer[i].step()
#
#                 obj_surrogates.append(obj_surrogate.item())
#                 obj_entropys.append(obj_entropy.item())
#
#         # self.iter_index += 1
#         # ep_ratio = 1. - (self.iter_index / self.total_iterations)
#         # self.ratio_clip = self.ratio_clip * ep_ratio
#         kl = []
#         for i, act in enumerate(self.act):
#             kl.append(torch.distributions.kl_divergence(tar_act[i].get_distribution(state),
#                                                         act.get_distribution(state)).mean().item())
#         self.update_record(obj_a0=obj_surrogates[0],
#                            obj_a1=obj_surrogates[1],
#                            obj_c=obj_critic.item(),
#                            mean_kl0=100 * kl[0],
#                            mean_kl1=100 * kl[1],
#                            entropy0=(-obj_entropys[0]),
#                            entropy1=(-obj_entropys[1]),
#                            a0_avg=buf_action[:, 0].mean().item(),
#                            a1_avg=buf_action[:, 1].mean().item(),
#                            a0_std=buf_action[:, 0].std().item(),
#                            a1_std=buf_action[:, 1].std().item(),
...
#
#     def load_model(self, cwd):
#         act_save_path = ['{}/actor0.pth'.format(cwd), '{}/actor1.pth'.format(cwd)]
...
#             network.load_state_dict(network_dict)
#
#         if (self.act is not None) and os.path.exists(act_save_path[0]):
#             load_torch_file(self.act_, act_save_path[0])
#             load_torch_file(self.act_safe, act_save_path[1])
#             print("Loaded act:", cwd)
#         if (self.cri is not None) and os.path.exists(cri_save_path):
#             load_torch_file(self.cri, cri_save_path)
#             print("Loaded cri:", cwd)
...
#     def to_cpu(self):
#         device = torch.device('cpu')
#         if isinstance(self.act, list):
#             for i in range(len(self.act)):
#                 self.act[i].to(device)
#         else:
#             self.act.to(device)
#         if isinstance(self.cri, list):
#             for i in range(len(self.act)):
#                 self.cri[i].to(device)
#         else:
#             self.cri.to(device)
#
#     def to_device(self):
#         if isinstance(self.act, list):
#             for i in range(len(self.act)):
#                 self.act[i].to(self.device)
#         else:
#             self.act.to(self.device)
#         if isinstance(self.cri, list):
#             for i in range(len(self.act)):
#                 self.cri[i].to(self.device)
#         else:
#             self.cri.to(self.device)
...
#             bs = 2 ** 10  # set a smaller 'bs: batch size' when out of GPU memory.
#             buf_value = torch.cat([self.cri(buf_state[i:i + bs]) for i in range(0, buf_state.size(0), bs)], dim=0)
#             buf_tar_loc, buf_tar_cholesky = self.act.get_loc_cholesky(buf_state)
#             buf_tar_pi = MultivariateNormal(loc=buf_tar_loc, scale_tril=buf_tar_cholesky)
#             buf_logprob = buf_tar_pi.log_prob(buf_action)
#             # buf_logprob = self.act.compute_logprob(buf_state, buf_action).unsqueeze(dim=1)
#             buf_r_ret, buf_adv = self.compute_reward(buf_len, buf_reward, buf_mask, buf_value)
#             tar_act = deepcopy(self.act)
#             del buf_reward, buf_mask
...
#             logprob = buf_logprob[indices]
#             adv = buf_adv[indices]
#             tar_loc = buf_tar_loc[indices]
#             tar_cholesky = buf_tar_cholesky[indices]
#
#             loc, cholesky = self.act.get_loc_cholesky(state)
#             # if torch.isnan(loc).any():
#             #     print()
#             # if torch.isinf(loc).any():
#             #     print()
#             fixed_stddev_dist = MultivariateNormal(loc, scale_tril=tar_cholesky)
#             fixed_mean_dist = MultivariateNormal(tar_loc, scale_tril=cholesky)
#             ratio = 0.5 * (fixed_stddev_dist.log_prob(action) - logprob).exp() + \
#                     0.5 * (fixed_mean_dist.log_prob(action) - logprob).exp()
#             obj_surrogate1 = adv * ratio
#             if self.objective_type == 'clip':
#                 obj_surrogate2 = adv * ratio.clamp(1 - self.ratio_clip, 1 + self.ratio_clip)
#                 obj_surrogate = -torch.min(obj_surrogate1, obj_surrogate2).mean()
...
#                                                             self.act.get_distribution(state)).mean()
#                 obj_surrogate = -obj_surrogate1.mean() + self.beta * mean_kl
#             new_logprob = MultivariateNormal(loc, scale_tril=tar_cholesky).log_prob(action)
#             obj_entropy = (new_logprob.exp() * new_logprob).mean()  # policy entropy
#             obj_actor = obj_surrogate + obj_entropy * self.lambda_entropy
#
#             self.act_optimizer.zero_grad()
...
#         # self.ratio_clip = self.ratio_clip * ep_ratio
#         kl = torch.distributions.kl_divergence(tar_act.get_distribution(state), self.act.get_distribution(state)).mean()
#         # topk = int(kl.shape[0] * 0.1)
#         self.update_record(obj_a=obj_surrogate.item(),
#                            obj_c=obj_critic.item(),
#                            mean_kl=100 * kl.mean().item(),
#                            max_kl=100 * kl.max().item(),
#                            # topk_kl=100 * (torch.topk(kl, topk).values.mean().item()),
#                            entropy=(-obj_entropy.item()))
#
#         return self.train_record


# class AgentSafePPO2(AgentBase):
#     def __init__(self, args=None):
#         super().__init__(args)
#         # could be 0.2 ~ 0.5, ratio.clamp(1 - clip, 1 + clip),
#         self.ratio_clip = 0.3 if args is None else args['ratio_clip']
...
#         self.compute_reward = None  # attribution
#
#     def init(self, net_dim, state_dim, action_dim, reward_dim=2, if_per=False, device=None):
#         self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu") if device is None else device
#         self.compute_reward = self.compute_reward_gae if self.if_use_gae else self.compute_reward_adv
#         self.safe_actions = [0., 0.2, 0.4, 0.6, 0.8, 1.]
#         self.cri = CriticAdv(state_dim, net_dim, self.if_use_dn).to(self.device)
#         self.cri_safe = QNetTwinDuel(net_dim, state_dim + action_dim, len(self.safe_actions)).to(self.device)
#         self.act_safe = self.cri_safe
#         if self.policy_type == 'mg':
#             self.act_ = ActorPPOMG(net_dim, state_dim, action_dim, self.if_use_dn).to(self.device)
#         else:
#             self.act_ = ActorPPO(net_dim, state_dim, action_dim, self.if_use_dn).to(self.device)
#         self.act = [self.act_, self.act_safe]
#         self.cri_optimizer = torch.optim.Adam(params=self.cri_safe.parameters(), lr=self.learning_rate)
#         self.crisafe_optimizer = torch.optim.Adam(params=self.cri.parameters(), lr=self.learning_rate)
#         self.act_optimizer = torch.optim.Adam(params=self.act_.parameters(), lr=self.learning_rate)
#
#         self.criterion = torch.nn.SmoothL1Loss()
#         self.iter_index = 0
#         assert if_per is False  # on-policy don't need PER
...
#         # return action
#         return np.hstack((action, steer_clip))
#
#     def update_net(self, buffer, _target_step, batch_size, repeat_times=4) -> (float, float):
#         buffer.update_now_len_before_sample()
#         buf_len = buffer.now_len  # assert buf_len >= _target_step
...
#         with torch.no_grad():
#             buf_reward, buf_mask, buf_action, buf_state = buffer.sample_all()
#             buf_r_v = torch.cat([self.cri(buf_state[i:i + bs]) for i in range(0, buf_state.size(0), bs)], dim=0)
#             buf_r_ret, buf_adv = self.compute_reward(buf_len, buf_reward[:, 0].unsqueeze(dim=1),
#                                                      buf_mask[:, 0].unsqueeze(dim=1), buf_r_v)
#             tar_act = deepcopy(self.act_)
#             buf_logprob = self.act_.compute_logprob(buf_state, buf_action).unsqueeze(dim=1)
#             buf_safe_aprob = torch.cat([self.cri_safe.get_a_prob(torch.cat((buf_state[i:i + bs],
#                                                                             buf_action[i:i + bs]), dim=1))
#                                         for i in range(0, buf_state.size(0), bs)], dim=0)
#             # buf_safe_a = buf_safe_aprob.max(dim=1, keepdim=True)[0].type(torch.long)
#             buf_safe_a = torch.multinomial(buf_safe_aprob, num_samples=1, replacement=True).type(torch.long)
#             tar_cri_safe = deepcopy(self.cri_safe)
#             del buf_r_v, buf_safe_aprob
#         '''PPO: Surrogate objective of Trust Region'''
#         for i in range(int(repeat_times * buf_len / batch_size)):
#             indices = torch.randint(buf_len - 1, size=(batch_size,), requires_grad=False, device=self.device)
#
...
#             adv = buf_adv[indices]
#
#             obj_critic_safe = self.get_obj_safe_critic(tar_cri_safe,
#                                                        buf_state,
#                                                        buf_action,
#                                                        buf_safe_a,
#                                                        buf_reward,
#                                                        buf_mask,
#                                                        indices)
#             self.crisafe_optimizer.zero_grad()
#             obj_critic_safe.backward()
#             self.crisafe_optimizer.step()
#             self.soft_update(tar_cri_safe, self.cri_safe, self.soft_update_tau)
#
#             value = self.cri(state)  # critic network predicts the reward_sum (Q value) of state
#             obj_critic = self.criterion(value, r_ret)
#             self.cri_optimizer.zero_grad()
...
#             self.cri_optimizer.step()
#
#             new_logprob = self.act_.compute_logprob(state, action).unsqueeze(dim=1)  # it is obj_actor
#             ratio = (new_logprob - logprob).exp()
#             obj_surrogate1 = adv * ratio
#             if self.objective_type == 'clip':
#                 obj_surrogate2 = adv * ratio.clamp(1 - self.ratio_clip, 1 + self.ratio_clip)
...
#             elif self.objective_type == 'kl':
#                 mean_kl = torch.distributions.kl_divergence(tar_act.get_distribution(state),
#                                                             self.act_.get_distribution(state)).mean()
#                 obj_surrogate = -obj_surrogate1.mean() + self.beta * mean_kl
#             obj_entropy = (new_logprob.exp() * new_logprob).mean()  # policy entropy
#             obj_actor = obj_surrogate + obj_entropy * self.lambda_entropy
#
...
#             self.act_optimizer.step()
#
#         kl = torch.distributions.kl_divergence(tar_act.get_distribution(state),
#                                                self.act_.get_distribution(state)).mean()
#         self.update_record(obj_a=obj_surrogate.item(),
#                            obj_c=obj_critic.item(),
#                            obj_s=obj_critic_safe.item(),
#                            mean_kl=100 * kl.mean().item(),
#                            # a_std=self.act.a_std_log.exp().mean().item(),
#                            entropy=(-obj_entropy.item()),
#                            a0_avg=buf_action[:, 0].mean().item(),
...
#         self.act_.to(device)
#         self.cri.to(device)
#         self.cri_safe.to(device)
#
#     def to_device(self):
#         self.act_.to(self.device)
#         self.cri.to(self.device)
#         self.cri_safe.to(self.device)
#
#     def get_obj_safe_critic(self, tar_cri_safe, buf_state, buf_action, buf_safe_a, buf_reward, buf_mask, indices):
#         with torch.no_grad():
#             next_input = torch.cat((buf_state[indices + 1], buf_action[indices + 1]), dim=1)
#             curr_input = torch.cat((buf_state[indices], buf_action[indices]), dim=1)
#             next_q = torch.min(*tar_cri_safe.get_q1_q2(next_input)).max(dim=1, keepdim=True)[0]
#             safe_q = buf_reward[indices, 1].unsqueeze(dim=1) + buf_mask[indices, 1].unsqueeze(dim=1) * next_q
#         q1, q2 = [qs.gather(1, buf_safe_a[indices]) for qs in self.cri_safe.get_q1_q2(curr_input)]
#         obj_critic_safe = self.criterion(q1, safe_q) + self.criterion(q2, safe_q)
#         return obj_critic_safe
#
#     def compute_reward_adv(self, buf_len, buf_reward, buf_mask, buf_value) -> (torch.Tensor, torch.Tensor):
#         """compute the excepted discounted episode return
#
...
#
#         self.cri = CriticAdv(state_dim, net_dim, self.if_use_dn).to(self.device)
#         self.cri_action = CriticAdv(state_dim, net_dim, self.if_use_dn).to(self.device)
#         if self.policy_type == 'mg':
#             self.act = ActorPPOMG(net_dim, state_dim, action_dim, self.if_use_dn).to(self.device)
#         else:
#             self.act = ActorPPO(net_dim, state_dim, action_dim, self.if_use_dn).to(self.device)
#         self.cri_optimizer = torch.optim.Adam(params=self.cri.parameters(), lr=self.learning_rate)
#         self.act_optimizer = torch.optim.Adam(params=self.act.parameters(), lr=self.learning_rate)
#         self.ca_optimizer = torch.optim.Adam(self.cri_action.parameters(), lr=self.learning_rate)
#
#         self.criterion = torch.nn.SmoothL1Loss()
#         self.iter_index = 0
#         assert if_per is False  # on-policy don't need PER
...
#         '''Trajectory using reverse reward'''
#         with torch.no_grad():
#             buf_reward_origin, buf_mask, buf_action, buf_state = buffer.sample_all()
#             buf_value = torch.cat([self.cri(buf_state[i:i + bs]) for i in range(0, buf_state.size(0), bs)], dim=0)
#             buf_cvalue = torch.cat([self.cri_action(buf_state[i:i + bs]) for i in range(0, buf_state.size(0), bs)],
#                                    dim=0)
#             buf_ra_ret, _ = self.compute_reward(buf_len, buf_reward_origin[:, 1].unsqueeze(dim=1),
#                                                 buf_mask[:, 1].unsqueeze(dim=1), buf_cvalue)
#             buf_cvalue_tmp = torch.zeros(buf_cvalue.shape, dtype=torch.float32, device=self.device)
#             buf_cvalue_tmp[:-1] = buf_cvalue[1:]
#             buf_f = buf_mask[:, 0].unsqueeze(dim=1) * buf_cvalue_tmp - buf_cvalue
#             buf_reward = buf_reward_origin[:, 0].unsqueeze(dim=1) + buf_f
#             # buf_reward = buf_reward_origin[:, 0].unsqueeze(dim=1)
#             buf_r_ret, buf_adv = self.compute_reward(buf_len, buf_reward,
#                                                      buf_mask[:, 0].unsqueeze(dim=1), buf_value)
#             tar_act = deepcopy(self.act)
#             buf_logprob = self.act.compute_logprob(buf_state, buf_action).unsqueeze(dim=1)
#             del buf_reward, buf_mask, buf_f, buf_cvalue, buf_value
#         '''PPO: Surrogate objective of Trust Region'''
#         for _ in range(int(repeat_times * buf_len / batch_size)):
#             indices = torch.randint(buf_len, size=(batch_size,), requires_grad=False, device=self.device)
#
...
#             action = buf_action[indices]
#             r_ret = buf_r_ret[indices]
#             ra_ret = buf_ra_ret[indices]
#             logprob = buf_logprob[indices]
#             adv = buf_adv[indices]
#
#             cvalue_a = self.cri_action(state)  # critic network predicts the reward_sum (Q value) of state
#             obj_a_critic = self.criterion(cvalue_a, ra_ret)
#             self.ca_optimizer.zero_grad()
#             obj_a_critic.backward()
...
#         self.act.to(device)
#         self.cri.to(device)
#         self.cri_action.to(device)
#
#     def to_device(self):
#         self.act.to(self.device)
#         self.cri.to(self.device)
#         self.cri_action.to(self.device)
#
#     def compute_reward_adv(self, buf_len, buf_reward, buf_mask, buf_value) -> (torch.Tensor, torch.Tensor):
#         """compute the excepted discounted episode return
#
...
#                 self.weights = None
#             self.pid_Ki = args['pid_Ki'] if 'pid_Ki' in args.keys() else 0.01
#             self.pid_Kp = args['pid_Kp'] if 'pid_Kp' in args.keys() else 1
#             self.pid_Kd = args['pid_Kd'] if 'pid_Kd' in args.keys() else 4
#
#         self.cris = []
#         self.noise = None
...
#             self.cris.append(CriticAdv(state_dim, net_dim, self.if_use_dn).to(self.device))
#         self.act = ActorPPO(net_dim, state_dim, action_dim, self.if_use_dn).to(self.device)
#
#         self.optimizer = torch.optim.Adam([{'params': self.act.parameters(), 'lr': self.learning_rate}] +
#                                           [{'params': self.cris[i].parameters(), 'lr': self.learning_rate} for i in
#                                            range(len(self.cris))])
#
#         self.criterion = torch.nn.SmoothL1Loss()
#         assert if_per is False  # on-policy don't need PER
...
#             bs = 2 ** 10  # set a smaller 'bs: batch size' when out of GPU memory.
#             buf_value = torch.cat(
#                 [torch.cat([cri(buf_state[i:i + bs]) for i in range(0, buf_state.size(0), bs)], dim=0) \
#                  for cri in self.cris], dim=1)
#             buf_logprob = self.act.compute_logprob(buf_state, buf_action).unsqueeze(dim=1)
#             buf_r_ret, buf_adv = self.compute_reward(buf_len, buf_reward, buf_mask, buf_value)
#             if self.if_auto_weights:
#                 self.auto_weights(buf_r_ret)
...
#             obj_actor = obj_surrogate + obj_entropy * self.lambda_entropy
#
#             values = [cri(state) for cri in self.cris]  # critic network predicts the reward_sum (Q value) of state
#             obj_critics = [self.criterion(values[i].squeeze(1), r_ret[:, i]) for i in range(len(values))]
#
#             obj_critic = 0
...
#         device = torch.device('cpu')
#         self.act.to(device)
#         [self.cris[i].to(device) for i in range(len(self.cris))]
#
#     def to_device(self):
#         self.act.to(self.device)
#         [self.cris[i].to(self.device) for i in range(len(self.cris))]
#
#     def compute_reward_adv(self, buf_len, buf_reward, buf_mask, buf_value) -> (torch.Tensor, torch.Tensor):
#         """compute the excepted discounted episode return
#
...
#         nagetive_r_mean = buf_ret.mean(dim=0)[1:]
#         # PID update here: CRL :cost instead nagetive_r
#         cost_mean = -nagetive_r_mean
#         delta = cost_mean - self.cost_threshold  # ep_cost_avg: tensor
#         zeros = torch.zeros(cost_mean.shape, dtype=torch.float32, device=self.device)
#         self.pid_i = torch.max(zeros, self.pid_i + delta * self.pid_Ki)
#         a_p = self.pid_delta_p_ema_alpha
