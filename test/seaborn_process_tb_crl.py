...
if __name__ == '__main__':
    ...
    for label in ['return_0', 'return_1', 'reward_2', 'reward_3', 'reward_4']:
        ...
        for i in range(1, len(file_name) + 1):
            ...
            y = np.array(data_mean.value)
            x = np.array(data_mean.step)
            for data in [data_max, data_min]:
                y = np.hstack((y, np.array(data.value)))
                x = np.hstack((x, np.array(data.step)))
            if label in ['reward_2', 'reward_3', 'reward_4']:
                y = -y
            tmp_df = pd.DataFrame(np.vstack((x, y)).T)
            ylabel = "ReturnAll" if label in ['return_0'] else ylabel
            ylabel = "Return" if label in ['return_1'] else ylabel
            ylabel = "AvgCost: $\Delta Steer$" if label in ['reward_2'] else ylabel
            ylabel = "AvgCost: $Comfort$" if label in ['reward_3'] else ylabel
            ylabel = "AvgCost: $Lat Distance$" if label in ['reward_4'] else ylabel
            # xlabel = "Time(s)"
            ...
            #              data=tmp_df)
        df = df.reset_index()
        if label in ['reward_2']:
            d = 0.02
            plt.hlines(d, 0, 2000000, colors='r', linestyles='dashed')
            plt.text(1000000, d*1.2, f'{d}', color='r', fontsize=14)
        if label in ['reward_3']:
            d = 0.05
            plt.hlines(d, 0, 2000000, color='r', linestyles='dashed')
            plt.text(1000000, d*0.8, f'{d}', color='r', fontsize=14)
        if label in ['reward_4']:
            d = 0.12
            plt.hlines(d, 0, 2000000, colors='r', linestyles='dashed')
            plt.text(1000000, d*1.2, f'{d}', color='r', fontsize=14)
